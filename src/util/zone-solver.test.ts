...
import { buildCornerGrid, buildCrosserGrid, findTileByCorners, paintTerrainTiles, solveArea } from "./zone-solver.js";
...
function makeTileset(): TilesetInfo {
  return {
    ...
    terrainTypes: [
      ...
      { index: 1, name: "Water", rawName: "Water", strref: -1 },
      { index: 2, name: "Trees", rawName: "Trees", strref: -1 },
    ],
...
function makeForestPitTileset(): TilesetInfo {
  return {
    resref: "forestpit",
    displayName: "Forest Pit",
    interior: false,
    hasHeightTransition: false,
    envMap: "",
    transition: 0,
    ...
    terrainTypes: [
      ...
      { index: 1, name: "Pit", rawName: "Pit", strref: -1 },
    ],
    crosserTypes: [],
    primaryRules: [
      { placed: "Forest", placedHeight: 0, adjacent: "Pit", adjacentHeight: 0, changed: "Pit", changedHeight: 0 },
      { placed: "Pit", placedHeight: 0, adjacent: "Forest", adjacentHeight: 0, changed: "Forest", changedHeight: 0 },
    ],
    secondaryRules: [],
    groups: [],
    tiles: [
      makeTile(0, "Forest", "Forest", "Forest", "Forest"),
      makeTile(1, "Pit", "Pit", "Pit", "Pit"),
      makeTile(2, "Forest", "Forest", "Pit", "Forest"),
      makeTile(3, "Pit", "Forest", "Pit", "Forest"),
    ],
  };
}
  ...
  it("does not inject alien terrain when zones have incompatible adjacency", () => {
    ...
    const tileset: TilesetInfo = {
      ...
      terrainTypes: [
        ...
        { index: 2, name: "CastleWall", rawName: "CastleWall", strref: -1 },
      ],
      tiles: [
        makeTile(0, "Grass", "Grass", "Grass", "Grass"),
    ...
    for (const p of result.placements) {
      ...
      const hasGrass = [corners.topLeft, corners.topRight, corners.bottomLeft, corners.bottomRight].some(
        (c) => c.toLowerCase() === "grass",
      );
...
describe("paintTerrainTiles", () => {
  it("re-solves adjacent tiles sharing a painted pit tile's corners", () => {
    const tileset = makeForestPitTileset();
    const grid = Array.from({ length: 6 * 6 }, () => ({ tileId: 0, orientation: 0 }));

    const result = paintTerrainTiles(6, 6, tileset, grid, "Pit", [{ x: 2, y: 5 }]);

    expect(result.warnings).toHaveLength(0);
    expect(result.placements.map((p) => [p.x, p.y, p.tileId, p.orientation])).toEqual([
      [1, 4, 2, 2],
      [2, 4, 3, 3],
      [3, 4, 2, 3],
      [1, 5, 3, 2],
